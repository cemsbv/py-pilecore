...
def test_cur236_defaults():
    norm = norms.Cur236()
    assert norm.version == norms.CUR236_version.V2023 == "2023"
    assert str(norm) == "CUR236 (2023)"
...
def test_norms_serialize_defaults():
    ...
    assert payload["CUR236_version"] == norms.CUR236_version.V2023 == "2023"


def test_norms_accept_custom_versions():
    norm = norms.Norms(
        nen_9997_1=norms.NEN99971_version.V2017,
        cur_236=norms.CUR236_version.V2023,
    )
    payload = norm.serialize_payload()
