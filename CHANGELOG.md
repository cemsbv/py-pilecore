...
### Bug Fixes
- *Input*: Stop sending undocumented uplift payload properties

### Features
- *Input*: Allow omitting safety factors from payloads

### Miscellaneous Tasks
- *Gitignore*: Ignore local .scratch issue tracker
- *Notebooks*: Strip outputs and refresh kernel metadata

### Testing
- *Openapi*: Update PileCore-API openapi definition
...
- *Results*:
    - Guard tension bearing overview on optional cpt_table
    - Narrow optional soil data before plotting

### Documentation
- *Grouper*: End-to-end custom Grouper flow snippet
- *Grouper-custom-bearing*: Add feature context & ADR
- *Readme*: Fix lint script name and add no-Docker lint steps

### Features

- *Grouper*:
    - CustomBearingResults -> grouper payload
    - Source-agnostic GrouperResults + viewers
    - Source-agnostic payload seam
- *Soil*: Coordinate-only SoilProperties

### Miscellaneous Tasks
- Bootstrap Claude Code agent setup

### Styling
- Apply black and isort to grouper and results modules

### Testing

- *Grouper*:
    - Tier-2 soil trace unlocks overview plots
    - Full pipeline + equivalence via CustomBearingResults

### Build
- *Deps*: Sync lint deps to super-linter v8, prune redundant
...
### Bug Fixes
- Include master negative_shaft_friction in per-CPT manual friction payload (#262)

## [2.3.0] - 2026-04-13

### Bug Fixes
- Strip construction_sequence from payload for CUR236, not NEN9997-1

## [2.2.3] - 2026-03-02

### Bug Fixes
- *Tests*: Correct response payloads in tests
- Revert input signature of alpha_t_clay and alpha_s_clay + update payload serialization.

## [2.2.2] - 2026-02-23

### Bug Fixes
- Only allow height_base=None for piles with anchor (CUR236) reference

## [2.2.1] - 2026-02-20

### Bug Fixes
- Re-allow base-height=None for piles, allowing for anchor piles. The pile shape is now correctly visualised in the geometry side-view plot methods

## [2.2.0] - 2026-02-19

### Documentation
- Add missing argument docstrings

### Features
- *Uplift*: #237 | Infer uplift endpoint from payload if possible

## [2.1.0] - 2026-02-16

### Features
- Conform to PileCore-api 4.1.0

### Miscellaneous Tasks
- *Lint*: Deactivate Spell Codespell in GH Superlinter
...
### Miscellaneous Tasks
- External notebook image references
...
### Bug Fixes
- Soil code key mappings
- Create_grouper_payload
- Grouper notebook fixup

### Documentation
- Update grouper notebook markdown
- Fix docstrings

## [2.0.0-beta.0] - 2026-01-20
...
- *Notebook*:
    ...
    - Update static images for pile reference tables
...
- *Grouper results*:
    ...
    - Add pypilecore.results.CasesGrouperResults class
...
- *Format*:
    - Lint & format
    - Lint and format
- *Notebook*: Notebooks for local/staging testing

- *Refactor*:
    - Units Enum
    - Rename ResultDefinition Enum classes
- DRY plot_bearing_overview
- Update github workflow Test docs

### Refactor
- *Api*: Rename standard_pile to reference
- *Notebook*: Update grouper notebook to v4
- Rename file
- Merge plan-view Viewers for Grouper and MultiCpt results into single object
- Implement CptResultsTable object in favor of pandas.DataFrame property

### Revert
- Remove grouper/optimize_groups references
